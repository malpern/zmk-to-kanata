...
from converter.models import KeymapConfig, Layer, Binding, HoldTap, MacroBehavior
...
def test_extract_multiple_layers():
    ...
    assert {l.name for l in config.layers} == {
        "default_layer", "lower_layer", "raise_layer"
    }
    assert all(len(l.bindings) == 2 for l in config.layers)
